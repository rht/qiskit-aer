...
#define QV_MAX_GRID_SIZE 0xFFFFFFFFull  // Max grid size (work-items) for HIP/CUDA
      ...
      if (nt > 0) {
        ...
        if (nb <= max_blocks_per_launch) {
          ...
          while (offset < ntotal) {
            ...
            uint_t launch_threads = (remaining > threads_per_launch) ? threads_per_launch : remaining;
            uint_t launch_blocks = (launch_threads + nt - 1) / nt;
            dev_apply_function_with_offset<data_t, Function><<<launch_blocks, nt, 0, strm>>>(func, launch_threads, offset);
            offset += launch_threads;
          }
        }
      }
